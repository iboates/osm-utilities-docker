...
do
  ...
  if docker run --pull=never --rm osmium:$VERSION 2>&1 | grep -q "osmium"; then
    ...
    docker tag osmium:$VERSION iboates/osmium:$VERSION$SUFFIX
    if docker push iboates/osmium:$VERSION$SUFFIX; then
      record_pushed_tag "iboates/osmium:$VERSION$SUFFIX"
